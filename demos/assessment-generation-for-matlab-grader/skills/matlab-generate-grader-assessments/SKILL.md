...
An item folder contains `description.txt`, `description.html`, `solution.m`, `template.m`, and `assessments.md`; for Function, Class Definition, Class Inheritance, and Class Methods items, it also contains `function_call.m`. `description.txt` is a simplified plain-text fallback. `description.html` is the preferred source for the MATLAB Grader Description and Instructions field because instructors can open it in a browser and copy rendered rich text. `tests.m` is created only when the item needs one or more **MATLAB Code** assessments. Referenced class, helper, and data files must be human-readable source or data files and must be documented in `assessments.md`; do not generate `.p` files.
...
- The file is an instructor-facing companion, not a replacement for the native item folders. Include each generated item’s title, rich-description paste instructions that link to `description.html`, submission type, referenced files, reference solution, learner template, template line-lock setup, assessment setup, and only optional feedback entries supported by validated incorrect variants. When the item is Function, Class Definition, Class Inheritance, or Class Methods, also include the standard field **How to call the function (when the learner clicks 'Run')** containing the `function_call.m` content.
- Do not put `description.txt` content in a fenced `Copy` block in combined output. Instead, link to the item’s `description.html` file and instruct the instructor to open it in a browser, click **Copy Description for MATLAB Grader**, and paste into the MATLAB Grader Description and Instructions field. Mention `description.txt` only as the plain-text fallback.
...
- Create both `description.txt` and `description.html`. The plain-text file must be readable if pasted directly into MATLAB Grader, but it must not contain Markdown backticks, fenced code blocks, raw HTML tags, Live Script `%[text]` markup, or other source markup that leaks literally in MATLAB Grader.
- Create `description.html` as a self-contained HTML file for browser-rendered copy. The file must include a button labeled **Copy Description for MATLAB Grader** that copies only the rendered description content, writing both `text/html` and `text/plain` clipboard data when the browser supports it. The copied `text/html` payload must be serialized from a cleaned clone of the description region with whitespace-only text nodes removed outside `pre`, `code`, and `textarea`; do not copy raw indented `innerHTML`, because MATLAB Grader can paste those formatting whitespace nodes as extra blank lines. For clipboard API failures, copy from a temporary hidden container populated with the same cleaned HTML payload rather than selecting the visible indented description region.
- In `description.html`, use semantic HTML for headings, paragraphs, ordered and unordered lists, bold, italic, underline, inline monospace, hyperlinks, and code example boxes. Use inline LaTeX delimited with `$...$` for equations. Do not use MathML, SVG equation images, or HTML superscript/subscript as the primary equation representation because they do not survive the MATLAB Grader paste path reliably.
...
6. Inspect `description.txt` and `description.html`. The plain-text description must have no source markup, and the HTML description must contain the copy button, a single rendered description region, and only supported rich-text patterns. The copy script must remove whitespace-only text nodes before writing or fallback-copying `text/html` to the clipboard. Equations must use `$...$` inline LaTeX.
7. If combined `AllGraderItems.md` output is generated, inspect every assessment `Copy` block. UI labels such as `Variable name:`, `Command or keyword:`, and `Code to paste:` must appear outside the fenced block only. For **Variable equals reference solution**, any fenced assessment field block containing anything other than the single variable identifier fails generation. The Description and Instructions section must link to `description.html` instead of containing a fenced description `Copy` block.
8. In an operating-system temporary directory outside the repository and instructor-facing item folder, create a class-based `matlab.unittest` harness. Use `matlab-testing` and run it through MATLAB MCP against the reference solution, a completed learner template, and targeted incorrect variants. Confirm the reference and completed template pass, concept-specific mutants fail, every requirement in the matrix is represented, and every nonempty feedback entry is backed by its linked mutant.
9. Do not leave the transient harness in the instructor-facing item folder. Report the MCP run result and its limits: it validates MATLAB behavior and the documented configuration model, while the instructor still pastes/configures the rows in MATLAB Grader.
